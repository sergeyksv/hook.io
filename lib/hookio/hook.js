...
Hook.prototype.emit = function (event, data, callback) {
  ...
  //
  // Remark: Hardcode absense of callbacks for events
  //
  callback = null;
  ...
  self.toIPs(self['hook-host'], function onResolve (err, hosts) {
    ...
    self.server = dnode(function (client, conn) {
      ...
      this.report = function (hook, cb) {
        ...
        cb(client.name, null, formatHandShake(self.pkginfo));
      };
      ...
      self.onAny(function (data, callback) {
        ...
        var loop = false;
        parts.forEach(function(pname) {
            if (pname==client.name)
        	loop = true;
        })
        if ((client.message) && !loop) {
          ...
          //  TODO: Sending of events to all clients regardles of subscriptions
          //  status is bad things. But use rpc call before sending message is worse
          //  and with buggy dnode cause memory leaks
...
Hook.prototype.connect = function (options, callback) {
  ...
  client.connect(dnodeOptions, function (remote, conn) {
    ...
    remote.report(self.getInfo(),  function (newName, newID, pkginfo) {
      self.name = newName;
      self.id   = newID;
